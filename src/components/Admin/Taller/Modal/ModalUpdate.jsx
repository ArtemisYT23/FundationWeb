...
export const TallerUpdateModal = () => {
  const dispatch = useDispatch();
  const styless = useStyless();
  const { tallerState } = useSelector((store) => store);
  const { TallerUpdate, TallerSelect } = tallerState;
  ...
  useEffect(() => {
    setTitle(TallerSelect?.title);
    setReview(TallerSelect?.review);
    setRecomendation(TallerSelect?.recommendation);
    setDateTaller(TallerSelect?.dateTaller);
  }, [TallerSelect]);
  ...
  const ComentaryView = (
    <div className={styless.UpdateComentary}>
      <form onSubmit={handleSubmit}>
        <div align="center">
          <TitleModal>Actualizar Taller</TitleModal>
        </div>
        <br />
        <TextField
          value={title}
        ...
        <div align="right">
          {title != "" &&
            recomendation != "" &&
            review != "" &&
            dateTaller != "" && <SaveButton>Actualizar</SaveButton>}
  ...
  return (
    <div className={styless.container}>
      <Modal open={TallerUpdate} onClose={abrirCerrarModal}>
        {ComentaryView}
      </Modal>
    </div>
  );
